...
const CONFIG = Object.freeze({
  ...
  FOLDER_NAMES: Object.freeze({ ROOT: 'EAC Reports', PENDING: 'Pending Scans', PROCESSED: 'Processed Scans', REVIEW: 'Needs Review', DUPLICATE: 'Duplicate Scans', FAILED: 'Failed Scans' }),
  RUN_LOG_SHEET_NAME: 'ðŸ“Š EAC Run Log',
...
const EAC_CHECKER_SCHEDULE = Object.freeze([
]);
/* =========================================================================
 ...
 * arrives without one (see readEACFormFromImage_).
 * ========================================================================= */
 
const EAC_SCHEDULE = Object.freeze([
  { day: 'MONDAY', slots: [
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('ðŸ“‹ EAC Tools')
    .addItem('Scan Pending EAC Reports', 'scanEACPaperReports')
    .addItem('â° Install Hourly Auto-Trigger', 'setupTimeDrivenTrigger')
    .addItem('Sort Sheet Now', 'sortEACSheetNow')
    ...
    .addItem('ðŸ“Š Generate Monthly Summaries & Charts', 'generateEACSummaryNow')
...
function removeAllRowGroupsNow() {
  ...
  ss.toast("All row groups and '+' buttons removed permanently.", 'EAC Tools', 3);
}
 
function setupTimeDrivenTrigger() {
  ScriptApp.getProjectTriggers().forEach((trigger) => {
    if (trigger.getHandlerFunction() === 'scanEACPaperReports') {
      ScriptApp.deleteTrigger(trigger);
    }
  });
 
  ScriptApp.newTrigger('scanEACPaperReports').timeBased().everyHours(1).create();
  SpreadsheetApp.getActiveSpreadsheet().toast('1M trigger installed for EAC scans.', 'EAC Tools', 5);
}
...
function sortEACSheetNow() {
  ...
  ss.toast('EAC Sheet sorted.', 'EAC Tools', 3);
}
 ...
 */
function scanEACPaperReports() {
  const startTime = Date.now();
  const folders = getEacFolders_();
 
  const apiKey = PropertiesService.getScriptProperties().getProperty('MISTRAL_API_KEY');
  if (!apiKey) {
    SpreadsheetApp.getActiveSpreadsheet().toast('Missing MISTRAL_API_KEY', 'EAC Error', 10);
    return;
  }
  ...
  finalizeEACRun_(ss, sheet, tally, startTime);
}
...
function processSingleFile_(file, ctx) {
  ...
  try {
    const parsed = readEACFormFromImage_(file, apiKey);
    ...
    console.error('EAC scan failed for "' + fileName + '":', err);
    safeMoveTo_(file, folders.error);
    return 'error';
  }
}
 ...
 */
function readEACFormFromImage_(file, apiKey) {
  ...
  if (!hasRecognizableInspector) {
    const scheduled = lookupEACScheduledChecker(parsed.date);
    if (scheduled) {
...
function appendParsedRoomsToSheet_(parsed, fileName, sheet, existingSignatures) {
  ...
  const rooms = expandRoomRangeEAC(parsed.room);
...
function finalizeEACRun_(ss, sheet, tally, startTime) {
  try {
    sortSheetByRoomNumber(sheet);
    styleHeaderRowGreen(sheet);
  } catch (err) {
  ...
  try {
    generateEACSummary(ss, sheet);
    ss.toast('âœ… EAC summaries created on new tabs.', 'Summary', 5);
  } catch (err) {
  ...
  ss.toast(
    'EAC Done. P:' + tally.processed + ' R:' + tally.review + ' D:' + tally.duplicate + ' E:' + tally.error,
    'Scan Complete',
    5
  );
}
 
/** Creates (if needed) and returns the EAC Reports folder tree. */
function getEacFolders_() {
...
function normalizeRoomCandidate_(roomStr) {
  ...
  const withoutLabels = cleaned.replace(/\b(ROOM|RM|BLDG|EAC)\b/gi, '').trim();
...
/** Strips prefixes like "Room"/"RM"/"EAC" and returns the bare number, or a "start-end" range. */
function extractPureRoomNumber(roomStr) {
  if (!roomStr) return '';

  const cleaned = String(roomStr).trim().replace(/\b(ROOM|RM|BLDG|EAC)\b/gi, '').trim();
  const normalized = normalizeRoomCandidate_(cleaned);
  if (!normalized) return '';

  const rangeMatch = normalized.match(/^(\d{3,4})[-–—](\d{3,4})$/);
  if (rangeMatch) return rangeMatch[1] + '-' + rangeMatch[2];
...
function expandRoomRangeEAC(roomStr) {
  if (!roomStr) return [''];
 
  const cleaned = extractPureRoomNumber(roomStr);
  const rangeMatch = cleaned.match(/^(\d+)\s*[-–—]\s*(\d+)$/);
  if (!rangeMatch) return [cleaned || String(roomStr).trim()];
 
  const start = parseInt(rangeMatch[1], 10);
  const end = parseInt(rangeMatch[2], 10);
  const pad = rangeMatch[1].length;
 
  if (start > end || (end - start) > CONFIG.MAX_ROOM_RANGE_SPAN) return [cleaned || String(roomStr).trim()];
...
function lookupEACScheduledChecker(dateStr) {
  ...
  const daySchedule = EAC_CHECKER_SCHEDULE.find((s) => s.day === DAY_NAMES_[parsedDate.getDay()]);
...
/** Picks the sheet the whole script operates on: "Logs", else "EAC", else whatever's active. */
function getMainSheet(ss) {
  const spreadsheet = ss || SpreadsheetApp.getActiveSpreadsheet();
  const sheet =
    spreadsheet.getSheetByName('Logs') ||
    spreadsheet.getSheetByName('EAC') ||
    spreadsheet.getActiveSheet() ||
    spreadsheet.getSheets()[0];
 
  return sheet || spreadsheet.insertSheet('EAC');
}
...
function ensureSheetHeadersEAC(sheet) {
...
function generateEACSummaryNow() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  generateEACSummary(ss, getMainSheet(ss));
  ss.toast('EAC summary generated.', 'EAC Tools', 3);
}
 
function generateEACSummary(ss, dataSheet) {
  ...
  removeExistingEACSummarySheets_(ss);
  months.forEach((monthKey) => {
    const tabName = 'EAC Summary - ' + monthKey;
    ...
    summarySheet.getRange(1, 1)
      .setValue('ðŸ¢ EAC BUILDING MONITORING (' + monthKey.toUpperCase() + ')')
      .setFontWeight('bold')
      .setFontSize(13)
      .setFontColor('#1B5E20');
 
    const dataMap = aggregateEACData(monthlyData[monthKey]);
    const nextRow = renderEACTableAndChart(summarySheet, dataMap, 2, 'EAC (' + monthKey + '): Total YES vs NO by Room');
    const questionData = aggregateEACQuestionData(monthlyData[monthKey]);
    renderEACQuestionCharts(summarySheet, questionData, nextRow + 2);
    summarySheet.autoResizeColumns(1, 5);
  });
 
  ss.toast('âœ… Summary tabs created for ' + months.join(', '), 'EAC Summary', 5);
}
 
function aggregateEACData(dataRows) {
  const map = {};
...
function aggregateEACQuestionData(dataRows) {
  const map = {};
...
function renderEACQuestionCharts(sheet, questionData, startRow) {
...
function renderEACTableAndChart(sheet, dataMap, startRow, chartTitle) {
...
function removeExistingEACSummarySheets_(ss) {
  try {
    const sheets = ss.getSheets();
    for (let i = 0; i < sheets.length; i++) {
      const name = sheets[i].getName();
      if (name.indexOf('EAC Summary - ') === 0) {
        try { ss.deleteSheet(sheets[i]); } catch (e) { /* ignore deletion failures */ }
      }
    }
  } catch (e) {
    console.error('Failed to remove existing EAC summary sheets:', e);
  }
}
