/*
 * =================================================================================
 * WAC CLASSROOM MONITORING — OCR & SCHEDULING SYSTEM
 * =================================================================================
 * Google Apps Script bound to the WAC monitoring spreadsheet.
 *
 * Pipeline:
 ...
 *
 * Setup: Project Settings → Script Properties → add MISTRAL_API_KEY.
 *
 ...
 * through the COLUMNS map below — never a bare index — so the sheet layout
...
const CONFIG = Object.freeze({
  ...
  // --- Batch run limits (per execution of scanWACPaperReports) ---
  MAX_FILES_PER_BATCH: 10,
  RUN_TIME_BUDGET_MS: 4 * 60 * 1000, // stop picking up new files after 4 minutes
  THROTTLE_MS: 1500,                  // pause between files to stay under API limits
 
  // --- Dates: single source of truth for the audit year. Update these two
  // values once a year instead of hunting for "26" / "2026" in the file. ---
  TARGET_YEAR_FULL: 2026,
  TARGET_YEAR_SHORT: '26',
 
  // --- Scoring ---
  MAX_SCORE_PER_AUDIT: 80, // 8 questions x 10 points
 
  // --- Room ranges, e.g. "301-305" on a form means 5 separate rooms ---
  ...
  RUN_LOG_SHEET_NAME: '📊 WAC Run Log',
 
  OFFICIAL_ROOMS: Object.freeze([
    '101', '102', '104 chem 1', '105 chem 2', '106 chem 3',
    '201 cisco', '203', '204 psych lab 2', '205 physics lab', '206',
    '207 bio-chem', '208 micro lab', '209 psych lab 1',
    '301 IT lab 1', '302 IT lab 2', '305', '306',
    '401', '402', '403', '404'
  ]),
...
/* =========================================================================
 * SHEET COLUMN LAYOUT — the single source of truth.
...
/* =========================================================================
 * WEEKLY CHECKER SCHEDULE — used to auto-fill the inspector when a form
 * arrives without one (see readWACFormFromImage_).
 * ========================================================================= */
 
const WAC_CHECKER_SCHEDULE = Object.freeze([
  { day: 'MONDAY', slots: [
    { time: '7:30-12:00', assigned: 'EBABACOL', partner: '' },
    { time: '1:00-5:00', assigned: 'BASLOT', partner: '' }
  ] },
  { day: 'TUESDAY', slots: [
    { time: '7:30-12:00', assigned: 'BASLOT', partner: '' },
    { time: '1:00-5:00', assigned: 'AMPLAYO', partner: '' }
  ] },
  { day: 'WEDNESDAY', slots: [
    { time: '7:30-10:30', assigned: 'BASLOT', partner: 'AMPLAYO' },
    { time: '10:30-12:00', assigned: 'BASLOT', partner: '' },
    { time: '1:00-5:00', assigned: 'EBABACOL', partner: '' }
  ] },
  { day: 'THURSDAY', slots: [
    { time: '7:30-12:00', assigned: 'ABSIN', partner: '' },
    { time: '1:00-5:00', assigned: 'BASLOT', partner: '' }
  ] },
  { day: 'FRIDAY', slots: [
    { time: '7:30-9:30', assigned: 'BASLOT', partner: '' },
    { time: '9:30-12:00', assigned: 'BASLOT', partner: 'BERMOY' },
    { time: '1:00-4:30', assigned: 'ARIZA', partner: '' }
  ] },
  { day: 'SATURDAY', slots: [
    { time: '7:30-12:00', assigned: 'RAZALO', partner: '' }
  ] }
]);
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('📋 WAC Tools')
    .addItem('Scan Pending WAC Reports', 'scanWACPaperReports')
    .addItem('⏰ Install Hourly Auto-Trigger', 'setupTimeDrivenTrigger')
    .addItem('Sort Sheet Now', 'sortWACSheetNow')
    .addItem('🗑️ Remove All Row Plus Buttons', 'removeAllRowGroupsNow')
    .addItem('📊 Clean Existing Room Numbers', 'cleanExistingRoomsInSheet')
    .addItem('📊 Generate Monthly Summaries & Charts', 'generateWACSummaryNow')
...
function removeAllRowGroupsNow() {
  ...
  ss.toast("All row groups and '+' buttons removed permanently.", 'WAC Tools', 3);
}
 
function setupTimeDrivenTrigger() {
  ScriptApp.getProjectTriggers().forEach((trigger) => {
    if (trigger.getHandlerFunction() === 'scanWACPaperReports') {
      ScriptApp.deleteTrigger(trigger);
    }
  });
 
  ScriptApp.newTrigger('scanWACPaperReports').timeBased().everyHours(1).create();
  SpreadsheetApp.getActiveSpreadsheet().toast('1M trigger installed for WAC scans.', 'WAC Tools', 5);
}
...
function sortWACSheetNow() {
  ...
  ss.toast('WAC Sheet sorted.', 'WAC Tools', 3);
}
 ...
 * the hourly trigger) — already-handled files have moved out of the pending
 * folder and won't be seen again.
 */
function scanWACPaperReports() {
  const startTime = Date.now();
  const folders = getWacFolders_();
 
  const apiKey = PropertiesService.getScriptProperties().getProperty('MISTRAL_API_KEY');
  if (!apiKey) {
    SpreadsheetApp.getActiveSpreadsheet().toast('Missing MISTRAL_API_KEY', 'WAC Error', 10);
    return;
  }
  ...
  for (let i = 0; i < pendingFiles.length; i++) {
    ...
    if (file.getMimeType().indexOf('image/') !== 0) {
      ...
      tally.error++;
      continue; // not something we can OCR — skip the throttle delay too
    }
  ...
  finalizeWACRun_(ss, sheet, tally, startTime);
}
 
/**
 * Handles one *image* file end-to-end: OCR → validate → append → file the
...
function processSingleFile_(file, ctx) {
  ...
  try {
    const parsed = readWACFormFromImage_(file, apiKey);
    ...
    console.error('WAC scan failed for "' + fileName + '":', err);
    safeMoveTo_(file, folders.error);
    return 'error';
  }
}
 ...
 */
function readWACFormFromImage_(file, apiKey) {
  ...
  if (!hasRecognizableInspector) {
    const scheduled = lookupWACScheduledChecker(parsed.date);
    if (scheduled) {
...
function appendParsedRoomsToSheet_(parsed, fileName, sheet, existingSignatures) {
  ...
  const rooms = expandRoomRangeWAC(parsed.room);
...
function finalizeWACRun_(ss, sheet, tally, startTime) {
  try {
    sortSheetByRoomNumber(sheet);
    styleHeaderRowGreen(sheet);
  } catch (err) {
  ...
  try {
    generateWACSummary(ss, sheet);
    ss.toast('✅ WAC summaries created on new tabs.', 'Summary', 5);
  } catch (err) {
  ...
  ss.toast(
    'WAC Done. P:' + tally.processed + ' R:' + tally.review + ' D:' + tally.duplicate + ' E:' + tally.error,
    'Scan Complete',
    5
  );
}
 
/** Creates (if needed) and returns the WAC Reports folder tree. */
function getWacFolders_() {
...
/* =========================================================================
 * STRICT MAPPING — rooms & inspectors
...
/** Matches if any official checker's name appears inside the OCR'd inspector text. */
...
function normalizeRoomCandidate_(roomStr) {
  ...
  const withoutLabels = cleaned.replace(/\b(ROOM|RM|BLDG|WAC)\b/gi, '').trim();
...
/** True if an MM-DD-YY date falls after the current month — a sign the OCR misread the date. */
...
/** Current month/year — used whenever a date can't be parsed at all. */
function defaultMonthKey_() {
  return MONTH_NAMES_[new Date().getMonth()] + ' ' + CONFIG.TARGET_YEAR_FULL;
}
...
/** Strips prefixes like "Room"/"RM"/"WAC" and returns the bare number, or a "start-end" range. */
function extractPureRoomNumber(roomStr) {
  if (!roomStr) return '';
 
  const cleaned = String(roomStr).trim().replace(/\b(ROOM|RM|BLDG|WAC)\b/gi, '');
 
  const rangeMatch = cleaned.match(/(\d{3,4})\s*[-–—]\s*(\d{3,4})/);
  if (rangeMatch) return rangeMatch[1] + '-' + rangeMatch[2];
...
function expandRoomRangeWAC(roomStr) {
  if (!roomStr) return [''];
...
function buildRowData(parsed, fileName, reviewReason, officialRoom, officialInspector) {
  ...
  const finalDateTime = (parsed.date || '01-01-' + CONFIG.TARGET_YEAR_SHORT) + (parsed.time ? ' ' + parsed.time : '');
...
function lookupWACScheduledChecker(dateStr) {
  ...
  const daySchedule = WAC_CHECKER_SCHEDULE.find((s) => s.day === DAY_NAMES_[parsedDate.getDay()]);
...
/** Picks the sheet the whole script operates on: "Logs", else "WAC", else whatever's active. */
function getMainSheet(ss) {
  const spreadsheet = ss || SpreadsheetApp.getActiveSpreadsheet();
  const sheet =
    spreadsheet.getSheetByName('Logs') ||
    spreadsheet.getSheetByName('WAC') ||
    spreadsheet.getActiveSheet() ||
    spreadsheet.getSheets()[0];
 
  return sheet || spreadsheet.insertSheet('WAC');
}
...
function ensureSheetHeadersWAC(sheet) {
...
function generateWACSummaryNow() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  generateWACSummary(ss, getMainSheet(ss));
  ss.toast('WAC summary generated.', 'WAC Tools', 3);
}
 
function generateWACSummary(ss, dataSheet) {
  ...
  if (!months.length) months = [defaultMonthKey_()];
 
  months.forEach((monthKey) => {
    const tabName = 'WAC Summary - ' + monthKey;
    ...
    summarySheet.getRange(1, 1)
      .setValue('🏢 WAC BUILDING MONITORING (' + monthKey.toUpperCase() + ')')
      .setFontWeight('bold')
      .setFontSize(13)
      .setFontColor('#1B5E20');
 
    const dataMap = aggregateWACData(monthlyData[monthKey]);
    const nextRow = renderWACTableAndChart(summarySheet, dataMap, 2, 'WAC (' + monthKey + '): Total YES vs NO by Room');
    const questionData = aggregateWACQuestionData(monthlyData[monthKey]);
    renderWACQuestionCharts(summarySheet, questionData, nextRow + 2);
    summarySheet.autoResizeColumns(1, 5);
  });
 
  ss.toast('✅ Summary tabs created for ' + months.join(', '), 'WAC Summary', 5);
}
 
function aggregateWACData(dataRows) {
  const map = {};
  dataRows.forEach((row) => {
    const room = String(row[COLUMNS.ROOM] || '').trim();
    if (!room) return;
...
function aggregateWACQuestionData(dataRows) {
  const map = {};
...
function renderWACQuestionCharts(sheet, questionData, startRow) {
...
function renderWACTableAndChart(sheet, dataMap, startRow, chartTitle) {
