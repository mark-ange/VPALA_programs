/*
 * =================================================================================
 * RNP CLASSROOM MONITORING â€” OCR & SCHEDULING SYSTEM
 * =================================================================================
 * Google Apps Script bound to the RNP monitoring spreadsheet.
 *
 * Pipeline:
...
const CONFIG = Object.freeze({
  ...
  FOLDER_NAMES: Object.freeze({ ROOT: 'RNP Reports', PENDING: 'Pending Scans', PROCESSED: 'Processed Scans', REVIEW: 'Needs Review', DUPLICATE: 'Duplicate Scans', FAILED: 'Failed Scans' }),
  RUN_LOG_SHEET_NAME: 'ðŸ“Š RNP Run Log',
  OFFICIAL_ROOMS: Object.freeze([
    // --- 1st Floor (100 Series) ---
    "101", "102", "105", "111", "113", "114", "115", "122", "125", "126", "127", "128", "129",
    
    // --- 2nd Floor (200 Series) ---
    "202", "203", "204", "205", "206", "208", "209", "210", "211", "212",
    "215", "216", "217", "218", "219", "220", "221", "222", "223", "224", "225",
    "229", "230", "231", "232", "235", "237", "239",

    // --- 3rd Floor (300 Series) ---
...
const RNP_CHECKER_SCHEDULE = Object.freeze([
]);
/* =========================================================================
 ...
 * arrives without one (see readRNPFormFromImage_).
 * ========================================================================= */
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('ðŸ“‹ RNP Tools')
    .addItem('Scan Pending RNP Reports', 'scanRNPPaperReports')
    .addItem('â° Install Hourly Auto-Trigger', 'setupTimeDrivenTrigger')
    .addItem('Sort Sheet Now', 'sortRNPSheetNow')
    ...
    .addItem('ðŸ“Š Generate Monthly Summaries & Charts', 'generateRNPSummaryNow')
...
function removeAllRowGroupsNow() {
  ...
  ss.toast("All row groups and '+' buttons removed permanently.", 'RNP Tools', 3);
}
 
function setupTimeDrivenTrigger() {
  ScriptApp.getProjectTriggers().forEach((trigger) => {
    if (trigger.getHandlerFunction() === 'scanRNPPaperReports') {
      ScriptApp.deleteTrigger(trigger);
    }
  });
 
  ScriptApp.newTrigger('scanRNPPaperReports').timeBased().everyHours(1).create();
  SpreadsheetApp.getActiveSpreadsheet().toast('1M trigger installed for RNP scans.', 'RNP Tools', 5);
}
...
function sortRNPSheetNow() {
  ...
  ss.toast('RNP Sheet sorted.', 'RNP Tools', 3);
}
 ...
 */
function scanRNPPaperReports() {
  const startTime = Date.now();
  const folders = getRnpFolders_();
 
  const apiKey = PropertiesService.getScriptProperties().getProperty('MISTRAL_API_KEY');
  if (!apiKey) {
    SpreadsheetApp.getActiveSpreadsheet().toast('Missing MISTRAL_API_KEY', 'RNP Error', 10);
    return;
  }
  ...
  finalizeRNPRun_(ss, sheet, tally, startTime);
}
...
function processSingleFile_(file, ctx) {
  ...
  try {
    const parsed = readRNPFormFromImage_(file, apiKey);
    ...
    console.error('RNP scan failed for "' + fileName + '":', err);
    safeMoveTo_(file, folders.error);
    return 'error';
  }
}
 ...
 */
function readRNPFormFromImage_(file, apiKey) {
  ...
  if (!hasRecognizableInspector) {
    const scheduled = lookupRNPScheduledChecker(parsed.date);
    if (scheduled) {
...
function appendParsedRoomsToSheet_(parsed, fileName, sheet, existingSignatures) {
  ...
  const rooms = expandRoomRangeRNP(parsed.room);
...
function finalizeRNPRun_(ss, sheet, tally, startTime) {
  try {
    sortSheetByRoomNumber(sheet);
    styleHeaderRowGreen(sheet);
  } catch (err) {
  ...
  try {
    generateRNPSummary(ss, sheet);
    ss.toast('âœ… RNP summaries created on new tabs.', 'Summary', 5);
  } catch (err) {
  ...
  ss.toast(
    'RNP Done. P:' + tally.processed + ' R:' + tally.review + ' D:' + tally.duplicate + ' E:' + tally.error,
    'Scan Complete',
    5
  );
}
 
/** Creates (if needed) and returns the RNP Reports folder tree. */
function getRnpFolders_() {
...
function normalizeRoomCandidate_(roomStr) {
  ...
  const withoutLabels = cleaned.replace(/\b(ROOM|RM|BLDG|RNP)\b/gi, '').trim();
...
/** Strips prefixes like "Room"/"RM"/"RNP" and returns the bare number, or a "start-end" range. */
function extractPureRoomNumber(roomStr) {
  if (!roomStr) return '';
 
  const cleaned = String(roomStr).trim().replace(/\b(ROOM|RM|BLDG|RNP)\b/gi, '');
...
function expandRoomRangeRNP(roomStr) {
  if (!roomStr) return [''];
...
function lookupRNPScheduledChecker(dateStr) {
  ...
  const daySchedule = RNP_CHECKER_SCHEDULE.find((s) => s.day === DAY_NAMES_[parsedDate.getDay()]);
...
/** Picks the sheet the whole script operates on: "Logs", else "RNP", else whatever's active. */
function getMainSheet(ss) {
  const spreadsheet = ss || SpreadsheetApp.getActiveSpreadsheet();
  const sheet =
    spreadsheet.getSheetByName('Logs') ||
    spreadsheet.getSheetByName('RNP') ||
    spreadsheet.getActiveSheet() ||
    spreadsheet.getSheets()[0];
 
  return sheet || spreadsheet.insertSheet('RNP');
}
...
function ensureSheetHeadersRNP(sheet) {
...
function generateRNPSummaryNow() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  generateRNPSummary(ss, getMainSheet(ss));
  ss.toast('RNP summary generated.', 'RNP Tools', 3);
}
 
function generateRNPSummary(ss, dataSheet) {
  ...
  removeExistingRNPSummarySheets_(ss);
  months.forEach((monthKey) => {
    const tabName = 'RNP Summary - ' + monthKey;
    ...
    summarySheet.getRange(1, 1)
      .setValue('ðŸ¢ RNP BUILDING MONITORING (' + monthKey.toUpperCase() + ')')
      .setFontWeight('bold')
      .setFontSize(13)
      .setFontColor('#1B5E20');
 
    const dataMap = aggregateRNPData(monthlyData[monthKey]);
    const nextRow = renderRNPTableAndChart(summarySheet, dataMap, 2, 'RNP (' + monthKey + '): Total YES vs NO by Room');
    const questionData = aggregateRNPQuestionData(monthlyData[monthKey]);
    renderRNPQuestionCharts(summarySheet, questionData, nextRow + 2);
    summarySheet.autoResizeColumns(1, 5);
  });
 
  ss.toast('âœ… Summary tabs created for ' + months.join(', '), 'RNP Summary', 5);
}
 
function aggregateRNPData(dataRows) {
  const map = {};
...
function aggregateRNPQuestionData(dataRows) {
  const map = {};
...
function renderRNPQuestionCharts(sheet, questionData, startRow) {
...
function renderRNPTableAndChart(sheet, dataMap, startRow, chartTitle) {
...
function removeExistingRNPSummarySheets_(ss) {
  try {
    const sheets = ss.getSheets();
    for (let i = 0; i < sheets.length; i++) {
      const name = sheets[i].getName();
      if (name.indexOf('RNP Summary - ') === 0) {
        try { ss.deleteSheet(sheets[i]); } catch (e) { /* ignore deletion failures */ }
      }
    }
  } catch (e) {
    console.error('Failed to remove existing RNP summary sheets:', e);
  }
}
